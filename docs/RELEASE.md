...
3. Install the native binary and shared libraries at `/opt/codaris/`, backend environment at `/etc/codaris/backend.env`, and systemd units in `deploy/`. Install `web/assets/fonts/DejaVuSans.ttf` and `DejaVuSans-LICENSE.txt` as `/opt/codaris/DejaVuSans.ttf` and `/opt/codaris/DejaVuSans-LICENSE.txt` for PDF exports. Configure PostgreSQL statement/parameter logging so credentials and verification material are not logged.
