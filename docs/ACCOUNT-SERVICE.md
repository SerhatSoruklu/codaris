...
Sessions are random 256-bit opaque cookies, HttpOnly, SameSite=Strict and Secure in production, expiring after 12 hours. Only token hashes are persisted for sessions. Mutation requests require an exact configured Origin and application/json, protecting against cross-origin form submission. Session IDs rotate on login; password reset/change revokes all sessions. Token consumption and account mutation are one database transaction with row locks. Verification/reset links carry tokens in a URL fragment; confirmation requires POST so mail scanners cannot consume them with GET. Recovery gives a generic response. Rate limits persist in PostgreSQL; reverse-proxy limits supplement them.
