...
Development defaults to frontend `127.0.0.1:8081`, API `127.0.0.1:8080` and local SMTP capture `127.0.0.1:1025`. Keep frontend ports and backend `CODARIS_ORIGIN` in sync; `check-env` checks the pair. Backend SMTP/authentication and database credentials are never frontend settings.
